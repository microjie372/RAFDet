...
class RAFDet_Backbone(nn.Module):
    """ Backbone for RAFDet"""

    def __init__(self, model_cfg, num_class, point_cloud_range, input_channels, **kwargs):
        super().__init__()
            ...
            if self.layer_types[k] == 'SA_Layer':
                ...
                self.SA_modules.append(
                    pointnet2_modules.PointnetSAModuleMSG_WithSampling(
                        ...
                        trans_enable=sa_config.TRANS_ENABLE[k],
                        density=sa_config.DENSITY[k],
                        cyv_aug=sa_config.CYV_AUG[k],
                        cyv_features=sa_config.CYV_FEATURES[k],
                        stride=sa_config.STRIDE[k],
                        aggregation_mlp=aggregation_mlp,
                        confidence_mlp=confidence_mlp,
                        num_features=sa_config.NUM_FEATURES[k],
                        num_head=sa_config.NUM_HEADS[k],
                        num_hidden_features=sa_config.NUM_HIDDEN_FEATURES[k],
                        num_layer=sa_config.NUM_LAYERS[k],
                        dropout=sa_config.DROPOUT[k],
    ...
    def forward(self, batch_dict):
        ...
        for i in range(len(self.SA_modules)):
            ...
            if self.layer_types[i] == 'SA_Layer':
                ...
                li_xyz, li_features, li_cls_pred = self.SA_modules[i](batch_dict, xyz_input, feature_input, li_cls_pred, ctr_xyz=ctr_xyz)
